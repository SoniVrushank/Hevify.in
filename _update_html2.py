# -*- coding: utf-8 -*-
"""Fix hamburger insertion using precise anchor-based approach."""
import os
import re
...
def add_hamburger(content):
    """Add hamburger before theme-toggle, anchored by the prev sibling pattern."""
    # Pattern: find nav-links closing </ul> followed by nav-shell </div> then theme-toggle
    # This is unambiguous since nav-links is the only child of nav-shell
    pattern = r'(</ul>\n    </div>\n)(    <button class="theme-toggle)'
    replacement = r'\1' + HAMBURGER_BTN + r'\2'
    return re.sub(pattern, replacement, content)

def is_root_homepage(fp):
    """True only for the root WEB.FILES\index.html"""
    # The root index.html has WEB.FILES\\index.html (or /index.html on Unix)
    normalized = fp.replace('/', '\\')
    return normalized.endswith('WEB.FILES\\index.html')
...
def process_file(fp, content):
    ...
    if 'nav-hamburger' not in content:
        new_content = add_hamburger(content)
